...
def click(pos):
    global lives, score, soundtrack, rocks_velocity, max_number_of_rocks, spawning_velocity
    global started
...
def draw(canvas):
    global time, started, rock_group, my_ship, missile_group, lives, score, soundtrack, explosion_group, missile_group
...
def rock_spawner():
    global rock_group, rocks_in_game, my_ship, score, rocks_velocity, max_number_of_rocks, lives, spawning_velocity
    if score % 15 == 0 and score > 0:
        rocks_velocity += .1
    if score % 30 == 0 and score > 0:
        max_number_of_rocks += 2
        spawning_velocity -= 200
    if score % 40 == 0 and score > 0:
        lives += 1
    if len(rock_group) <= max_number_of_rocks and started:
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_vel = [random.random() * rocks_velocity - .3, random.random() * rocks_velocity - .3]
        rock_avel = random.random() * .2 - .1
        if dist(rock_pos, my_ship.get_position()) > my_ship.get_radius() + 50:
...
timer = simplegui.create_timer(spawning_velocity, rock_spawner)
